...
class TestPaths:
    ...
    def test_get_manifest_path(self,mocker,channel_id, guild_id, expected_path_loc):
        main.EPHEMERAL_PATH = "./"
    ...
    def test_get_manifest_s3(self,mocker,channel_id, guild_id, expected_path_loc):
        main.EPHEMERAL_PATH = "./"
    ...
    def test_get_manifest_seal_path(self, mocker, channel_id, guild_id, expected_path_loc):
        main.EPHEMERAL_PATH = "./"
    ...
    def test_get_manifest_seal_s3(self, mocker, channel_id, guild_id, expected_path_loc):
        main.EPHEMERAL_PATH = "./"
...
class TestConfiguration:
    ...
    def test_successful_s3_init(self, monkeypatch, s3enabled, s3bucket, s3accesskey, s3secretkey, s3endpoint):
        # prep env
        monkeypatch.setenv("S3_ENABLED", s3enabled)
        monkeypatch.setenv("S3_BUCKET", s3bucket)
        monkeypatch.setenv("S3_ACCESS_KEY", s3accesskey)
        monkeypatch.setenv("S3_SECRET_KEY", s3secretkey)
        monkeypatch.setenv("S3_ENDPOINT", s3endpoint)
...
class TestMisc:
    ...
    async def test_last_message_id(self,monkeypatch,tmp_path,mocker):
        msg_id = 123456543453
        monkeypatch.setenv("EPHEMERAL_PATH", str(tmp_path.absolute()))
        channel_mock = mocker.Mock()
...
class TestSigning:
    def test_load_sign_key(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv('SIGN_KEY_PEM', str(key_file.absolute()))
...
class TestStorage:
    def test_write_to_storage_fail_encrypt_no_keys(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv('EPHEMERAL_PATH', str(tmp_path))
...
'''
        ...
        monkeypatch.setenv('GPG_KEY_DIR', str(tmp_path.absolute()))
